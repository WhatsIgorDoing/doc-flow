...
def test_full_flow_validate_then_organize(
    mock_get_org_service, mock_get_valid_service, test_db_manager, tmp_path
):
    ...
    valid_file = DocumentFile(
        Path("c:/docs/contract.pdf"), 1024, DocumentStatus.VALIDATED
    )
    valid_file.associated_manifest_item = MagicMock(
        document_code="DOC-1", revision="0", title="Contract"
    )
    ...
    real_validation_service._Use_case.execute = AsyncMock(
    ...
    real_org_service = OrganizationService(
        file_manager=mock_fs_manager, db_manager=test_db_manager
    )
    ...
    resp_val = client.post(
        "/api/validate",
        json={"manifest_path": str(d_manifest), "source_directory": str(d_source)},
    )
    ...
    resp_org = client.post(
        "/api/organize", json={"output_directory": "c:/out", "max_docs_per_lot": 10}
    )
