...
from app.infrastructure.repositories import ManifestRepository, FileRepository, FileSystemManager
...
def official_manifest_path():
    ...
    path = Path(r"c:\Development\doc-flow\docs\modelos de planilhas\LD-5290.00-22212-91A-CZ6-001_C 04_02.xlsx")
    if not path.exists():
...
async def test_import_official_manifest(official_manifest_path):
    ...
    found = next((i for i in items if "5900.0130869.25.2-CZ6-CV-QUA-0001" in i.document_code), None)
    assert found is not None, "Não encontrou o item 5900.0130869.25.2-CZ6-CV-QUA-0001"
    assert found.revision == "0" or found.revision == "", "Revisão incorreta" # Mock pode variar
