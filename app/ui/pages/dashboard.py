    ...
    async def run_validation(self):
        ...
        try:
            ...
            with ui.dialog() as dialog, ui.card().classes("w-full max-w-lg"):
                ...
                with ui.expansion("Detalhes Técnicos", icon="code").classes(
                    "w-full bg-red-50"
                ):
                    ...
                    with GlassCard().classes("w-full flex flex-col p-8 gap-8"):
                        ...
                        with ui.grid(columns=2).classes("w-full gap-8"):
                            ...
                            self.picker_manifest = FilePickerMolecule(
                                "Importar Manifesto (.xlsx)", self.pick_manifest
                            )
                        ...
                        with ui.row().classes(
                            "w-full items-center justify-between pt-6 border-t border-gray-200/60"
                        ):
                            # Status Indicator
                            with ui.row().classes("items-center gap-2"):
                                ui.spinner(size="sm").bind_visibility_from(
                                    self, "is_validating"
                                )
                                self.status_label = ui.label(
                                    "Selecione os arquivos para começar."
                                ).classes("text-gray-400 font-medium")

                            # Action Buttons
                            with ui.row().classes("gap-4"):
                                ui.button("Limpar", on_click=self.reset_state).props(
                                    "flat text-color=grey-7"
                                )
