    ...
    with ui.row().classes("w-full items-center justify-between mb-8"):
        ...
        with ui.column().classes("gap-2"):
            with ui.row().classes("items-center gap-3"):
                ...
                ui.label("Validação de Documentos").classes(
                    design.typo.h2 + " text-gray-900"
                )

            ui.label("Gerencie, valide e organize seus lotes de entrega.").classes(
                design.typo.body + " text-gray-500 pl-1"
            )

        # Right: System Status
        with ui.row().classes(
            "items-center gap-4 bg-white/50 p-2 rounded-2xl border border-white/60"
        ):
            # Environment Badge
            with ui.row().classes("items-center px-3 py-1 bg-gray-100 rounded-lg"):
                ui.label(settings.ENVIRONMENT.upper()).classes(
                    "text-xs font-bold text-gray-500 tracking-wider"
                )
