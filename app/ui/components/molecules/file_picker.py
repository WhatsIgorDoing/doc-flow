...
class FilePickerMolecule(ui.column):
    ...
    def __init__(self, label: str, on_pick: callable, is_folder: bool = False):
        ...
        # Container styles
        self.classes(
            "w-full border-2 border-dashed border-gray-300 rounded-xl p-8 items-center justify-center gap-6 transition-all duration-300"
        )
    ...
    def set_selected(self, path: str):
        ...
        with self:
            ...
            with ui.column().classes("items-center"):
                ui.label("Selecionado:").classes(
                    "text-xs text-gray-500 uppercase tracking-widest"
                )
                ui.label(path).classes(
                    "text-base font-medium text-gray-800 break-all text-center px-4"
                )
